...
Git 레포지토리로 지식베이스를 만들고, 지식을 정리하기 시작했습니다. 벡터DB도, 임베딩도, 별도의 검색 시스템도 없이 markdown 파일로 문서를 쓰고, 관련된 것끼리 링크를 걸었습니다. AI 에이전트가 읽고 참고하게 하려는 목적이었지만, 사람이 필요한 지식을 찾고 관리하기 쉽게 만들고 싶기도 했습니다.

Git 저장소에 Markdown 문서를 쌓고, 문서 간 교차 참조를 두며, AI 에이전트가 읽고 갱신 PR을 올리는 방식은 Andrej Karpathy가 소개한 [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)와 비슷합니다.

그러다 문득 궁금해졌습니다. **링크를 걸어두는 게 실제로 AI 탐색에 도움이 될까?** AI에게 주려고 만든 건데, 정작 그 링크가 AI가 문서를 찾는 데 도움이 되는지는 확인해본 적이 없었습니다.

그래서 실제로 운영 중인 문서를 대상으로 확인해봤습니다.
...
이번 실험에서는 도움이 되는 경우가 있었습니다. 링크 탐색을 제공했을 때 일부 모델은 검색만으로 탐색할 때보다 정답 문서를 더 많이 찾았습니다. 다만 그 효과는 모델마다 달랐고, 입력 토큰도 일관되게 줄지는 않았습니다.

이 결과를 보면, 지금 운영하는 지식베이스에서 AI 탐색 성능을 높이려고 문서 링크를 적극적으로 늘릴 필요는 없을 것 같습니다. 일부 모델에서는 도움이 됐지만, 그 효과가 일관되지는 않았습니다. 기존 링크는 유지하되, AI를 위해 링크를 더 촘촘히 다는 데까지 시간을 쓰지는 않으려 합니다.
