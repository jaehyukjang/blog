...
6/10까지는 백필 작업의 PUT 요청이 섞여 있어 수치가 높지만, 6/11은 백필이 끝난 뒤 순수 운영 수치입니다. Requests/GB가 6,016에서 1,731로 **71% 감소** — 같은 양의 데이터를 읽는 데 필요한 요청 수가 대폭 줄었습니다.

조회량이 날마다 다르기 때문에, 동일 조회량(100,000GB) 기준으로 비용을 환산하면:

| 지표 | Before (6/5~6/8 avg) | After (6/11) | 변화 |
|------|---------------------|-------------|------|
| Requests/GB | 6,016 | 1,731 | -71% |
| Requests per 100TB | 601.6M | 173.1M | -71% |
| 월간 request 비용 (추정) | ~$7,220/month | ~$2,080/month | **-$5,140/month** |
